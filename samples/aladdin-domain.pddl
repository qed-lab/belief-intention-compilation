...
(define (domain aladdin)
  ...
  (:predicates (alive ?character - character)
               ...
               (married ?character - character)  ;;; this is a midline comment! Ain't it terrible?
               (at ?character - character ?place - place)
               (in ?genie - genie ?magic-lamp - magic-lamp)
               (has ?character - character ?thing - thing)
               (loyal-to ?knight - character ?king - character)
  ...
  (:action slay
    :parameters   (?knight - female ?monster - character ?place - place)
  ...
  (:action marry
    :parameters   (?groom - character ?bride - character ?place - place)
  ...
  (:action fall-in-love
    :parameters   (?male - character ?female - character ?place - place)
    ...
    :effect       (and (loves ?male ?female)
                       ...
                       (loyal-to ?male ?female)
                       (intends ?male (alive ?female))))

  ;; A character delegates a goal to a subordinate.
  (:action order
    :parameters   (?king - character ?knight - character ?place - place ?objective - expression)
  ...
  (:action appear-threatening
    ...
    ;; genie resurrects a character
    (:action reanimate
      :parameters   (?genie - genie ?character - character ?place - place)
      :precondition (and (not (= ?genie ?character))
                         (alive ?genie)
                         (at ?genie ?place)
                         (at ?character ?place)
                         (not(alive ?character)))
      :effect       (alive ?character)
