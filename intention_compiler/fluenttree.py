...
class AbstractPredicate:
    def __init__(self, leaf):
        if isinstance(leaf, str):
            ...
            if not (leaf.is_leaf or (leaf.is_not and len(leaf.child_trees) == 1 and leaf.child_trees[0].is_leaf)):
            ...
            if (not leaf.is_not) and leaf.is_belief and len(leaf.child_trees) == 1:
                self.is_belief = True
                intended_predicate = leaf.child_trees[0]
            ...
            if leaf.is_not and not leaf.child_trees[0].is_intends:
                self.is_not = True
                self.identifier = leaf.child_trees[0].identifier
                self.parameters = leaf.child_trees[0].words[1:]
                self.arity = len(self.parameters)

            # E.g "not( intends ?agent (predicate ?x ?y ?z)) "
            if leaf.is_not and leaf.child_trees[0].is_intends  and len(leaf.child_trees[0].child_trees)==1:
                self.is_not = True
                self.is_intention = True
                intention = leaf.child_trees[0]
                intended_predicate = intention.child_trees[0]
                self.identifier = intended_predicate.identifier
                self.parameters = intention.words[1:] + intended_predicate.words[1:]
                self.arity = len(self.parameters)

            # E.g "intends ?agent (predicate ?x ?y ?z)"
            if (not leaf.is_not) and leaf.is_intends and len(leaf.child_trees)==1:
                self.is_intention = True
                intended_predicate = leaf.child_trees[0]
                self.identifier = intended_predicate.identifier
                self.parameters = leaf.words[1:] + intended_predicate.words[1:]
            ...
            if leaf.is_not and leaf.child_trees[0].is_intends and len(leaf.child_trees[0].child_trees)==0:
                self.is_not = True
                self.is_intention = True
                self.has_expression_parameter = True
                intention = leaf.child_trees[0]
            ...
            if (not leaf.is_not) and leaf.is_intends and len(leaf.child_trees)==0:
                self.is_intention = True
                self.has_expression_parameter = True
        ...
        if self.identifier == "intends" or self.identifier == "believes" or not self.children:
            ...
            if self.identifier == "intends":
                ...
                if len(self.children) == 1:
                    self.child_trees.append(FluentTree(self.children[0], depth + 1))
            ...
            for child in self.children:
                self.child_trees.append(FluentTree(child, depth + 1))
